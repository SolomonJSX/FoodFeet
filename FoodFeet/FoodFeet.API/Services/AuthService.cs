    ...
    BasketService basketService)
{
    public async Task<UserCheckedResponse> Login(LoginDTO dto, HttpContext context)
    {
        ...
        if (user is null) return new UserCheckedResponse(false, "Not Authorized!", null, null);
        var basket = await dbContext.Baskets.AsNoTracking().FirstOrDefaultAsync(b => b.UserId == user.Id);

        if (basket is null) return new UserCheckedResponse(false, "Basket not found", null, null);

        if (!BCrypt.Net.BCrypt.Verify(dto.Password, user.PasswordHash))
            return new UserCheckedResponse(false,"Invalid password!", null, null);
        ...
        return new UserCheckedResponse(true, "Successfully registered!", userResponse, token);
    }
    ...
    public async Task<UserCheckedResponse> Register(RegisterDTO dto, HttpContext context)
    {
        ...
        if (user is not null) return new UserCheckedResponse(false, "Already Exists!", null, null);

        if (dto.Password != dto.ConfirmPassword)
            return new UserCheckedResponse(false, "Password and ConfirmPassword are not equivalent", null, null);
